...
export default function Sidebar() {
  ...
  const b = (t: string) => `min-h-[44px] min-w-[44px] flex items-center justify-center p-2 rounded-xl duration-300 transition-all active:scale-95 ${p === t ? 'text-white bg-stone-800' : 'hover:bg-stone-900 hover:text-stone-200'}`
  const chatButtonClass = `min-h-[44px] min-w-[44px] flex items-center justify-center p-2 rounded-xl duration-300 transition-all active:scale-95 ${isToggled ? 'text-white bg-stone-800' : 'hover:bg-stone-900 hover:text-stone-200'}`
  ...
  return (
    <>
      ...
      <aside className={`left-0 bottom-0 w-screen md:w-fit md:h-screen fixed p-4 z-50 lg:flex transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div id='idk' className='w-64 md:order-2 h-full z-40 rounded-l-none rounded-2xl bg-stone-950 border border-l-transparent border-stone-900 hidden flex-col p-4 space-y-3 overflow-y-auto custom-scroll'>
          {chats?.chats.map((chat) => (
            <Link key={chat.id} to={`/chat/?chatId=${chat.id}`} className='p-2 min-h-[44px] flex items-center hover:text-stone-200 hover:bg-stone-900 w-full rounded-xl block text-sm truncate active:scale-95 transition-all' onClick={() => setIsMobileMenuOpen(false)}>
        ...
        <div id='idk0' className='md:w-20 md:order-1 h-full rounded-3xl bg-stone-950/50 backdrop-blur-xl border border-stone-900 text-stone-400 flex flex-col items-center justify-between py-4'>
          ...
          <nav className='flex md:flex-col items-center space-x-4 md:space-x-0 md:space-y-2 my-auto'>
            <Tooltip content={t('home')} delay={500}>
              <Link to='/' className={b('/')} aria-label={t('home')} onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <Tooltip content={t('toggleChats')} delay={500}>
              <button onClick={() => toggleC()} className={chatButtonClass} aria-label={t('toggleChats')}>
            ...
            <Tooltip content={t('tools')} delay={500}>
              <Link to='/tools' className={b('/tools')} aria-label={t('tools')} onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <Tooltip content={t('exam')} delay={500}>
              <Link to='/exam' className={b('/exam')} aria-label={t('exam')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
                  <path d="M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32L19.513 8.2Z" />
                </svg>
              </Link>
            </Tooltip>
            <Tooltip content={t('quiz')} delay={500}>
              <Link to='/quiz' className={b('/quiz')} aria-label={t('quiz')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
            ...
            <Tooltip content={t('planner')} delay={500}>
              <Link to='/planner' className={b('/planner')} aria-label={t('planner')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <Tooltip content={t('debate')} delay={500}>
              <Link to='/debate' className={b('/debate')} aria-label={t('debate')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
                  <path fillRule="evenodd" d="M4.848 2.771A49.144 49.144 0 0 1 12 2.25c2.43 0 4.817.178 7.152.52 1.978.292 3.348 2.024 3.348 3.97v6.02c0 1.946-1.37 3.678-3.348 3.97a48.901 48.901 0 0 1-3.476.383.39.39 0 0 0-.297.17l-2.755 4.133a.75.75 0 0 1-1.248 0l-2.755-4.133a.39.39 0 0 0-.297-.17 48.9 48.9 0 0 1-3.476-.384c-1.978-.29-3.348-2.024-3.348-3.97V6.741c0-1.946 1.37-3.68 3.348-3.97ZM6.75 8.25a.75.75 0 0 1 .75-.75h9a.75.75 0 0 1 0 1.5h-9a.75.75 0 0 1-.75-.75Zm.75 2.25a.75.75 0 0 0 0 1.5H12a.75.75 0 0 0 0-1.5H7.5Z" clipRule="evenodd" />
                </svg>
              </Link>
            </Tooltip>
            <Tooltip content={t('cards')} delay={500}>
              <Link to='/cards' className={b('/cards')} aria-label={t('cards')} onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <Tooltip content={t('profile')} delay={500}>
              <Link to='/profile' className={b('/profile')} aria-label={t('profile')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
                  <path fillRule="evenodd" d="M18.685 19.097A9.723 9.723 0 0021.75 12c0-5.385-4.365-9.75-9.75-9.75S2.25 6.615 2.25 12a9.723 9.723 0 003.065 7.097A9.716 9.716 0 0012 21.75a9.716 9.716 0 006.685-2.653zm-12.54-1.285A7.486 7.486 0 0112 15a7.486 7.486 0 015.855 2.812A8.224 8.224 0 0112 20.25a8.224 8.224 0 01-5.855-2.438zM15.75 9a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" clipRule="evenodd" />
                </svg>
              </Link>
            </Tooltip>
            <Tooltip content={t('report')} delay={500}>
              <Link to='/report/weekly' className={b('/report/weekly')} aria-label={t('report')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
            ...
            <Tooltip content={t('help')} delay={500}>
              <Link to='/help' className={b('/help')} aria-label={t('help')} onClick={() => setIsMobileMenuOpen(false)}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fillRule="evenodd" clipRule="evenodd" d="M12 2.25C6.61522 2.25 2.25 6.61522 2.25 12C2.25 17.3848 6.61522 21.75 12 21.75C17.3848 21.75 21.75 17.3848 21.75 12C21.75 6.61522 17.3848 2.25 12 2.25ZM8.5 9C8.5 8.17157 9.17157 7.5 10 7.5H14C14.8284 7.5 15.5 8.17157 15.5 9V10C15.5 10.8284 14.8284 11.5 14 11.5H10C9.17157 11.5 8.5 10.8284 8.5 10V9ZM8.5 13.5C8.5 12.6716 9.17157 12 10 12H14C14.8284 12 15.5 12.6716 15.5 13.5V14.5C15.5 15.3284 14.8284 16 14 16H10C9.17157 16 8.5 15.3284 8.5 14.5V13.5Z" fill="currentColor" />
                  <path d="M12 17C12.5523 17 13 17.4477 13 18C13 18.5523 12.5523 19 12 19C11.4477 19 11 18.5523 11 18C11 17.4477 11.4477 17 12 17Z" fill="currentColor" />
                </svg>
              </Link>
            </Tooltip>
          </nav>
