      ...
      <div className="md:flex items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 p-1.5">
        <div className="flex items-center space-x-4 p-1.5 hover:bg-white/5 duration-300 transition-all cursor-pointer rounded-xl min-w-fit h-fit">
          <div className="flex flex-col -space-y-0.5">
            <span className="text-xs">{t("modelToolbar.model")}</span>
        ...
        <div className="p-1.5 md:h-full hover:bg-white/5 rounded-xl flex items-center space-x-2 cursor-pointer min-w-fit h-fit" onClick={onPickFile} title={stagedFileName || t("modelToolbar.clickOrDrop")}>
          ...
          <div className="flex flex-col -space-y-0.5">
            <span className="text-xs">{stagedFileName ? t("modelToolbar.fileSelected") : t("modelToolbar.addFiles")}</span>
            <span className="text-sm">{stagedFileName || t("modelToolbar.clickOrDrop")}</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4 p-1.5 hover:bg-white/5 duration-300 transition-all cursor-pointer rounded-xl w-full sm:w-auto">
          <div className="flex flex-col -space-y-0.5 pr-2">
            <span className="text-xs">{t("modelToolbar.difficulty")}</span>
            <span className="text-sm font-semibold">{t("modelToolbar.intermediate")}</span>
          </div>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 15 12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9" />
          </svg>
