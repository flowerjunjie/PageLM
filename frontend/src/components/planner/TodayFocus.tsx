        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
            ...
            <div className="space-y-4">
                ...
                {activeTasks.length > 0 && (
                    <div>
                        <div className="text-zinc-300 text-sm mb-2">{t('todayFocus.currentlyWorkingOn')}:</div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <div className="text-blue-300 text-xs">
                                                {task.course} • {task.estMins} mins • {t('task.due')} {fmtTime(task.dueAt)}
                                            </div>
                                        </div>
                                        <button
                ...
                {todayTasks.length > 0 && (
                    <div>
                        <div className="text-zinc-300 text-sm mb-2">{t('todayFocus.dueToday')} ({todayTasks.length}):</div>
                        <div className="space-y-2">
                            ...
                            {todayTasks.length > 3 && (
                                <div className="text-zinc-400 text-xs text-center">
                                    +{todayTasks.length - 3} {t('todayFocus.moreTasksDueToday')}
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {urgentTasks.length > 0 && (
                    <div>
                        <div className="text-zinc-300 text-sm mb-2">{t('todayFocus.urgent')} (Next 24h):</div>
                        <div className="space-y-2">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                ...
                                                <div className="text-yellow-300 text-xs">
                                                    {task.course} • {t('todayFocus.dueIn', { hours: Math.round(hoursLeft) })} • {task.estMins} mins
                                                </div>
                                            </div>
                                            <button
                                                onClick={() => onStartSession(task.id)}
                ...
                {todayTasks.length === 0 && urgentTasks.length === 0 && activeTasks.length === 0 && (
                    <div className="text-center py-8 text-zinc-400">
                        ...
                        <div className="text-sm">{t('todayFocus.nothingUrgentToday')}</div>
                        <div className="text-xs text-zinc-500">{t('todayFocus.greatJobStayingOnTop')}</div>
                    </div>
                )}
            </div>
        </div>
